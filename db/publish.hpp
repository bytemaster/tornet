...
namespace tornet { namespace db {
  ...
  class publish {
    public:
      typedef boost::shared_ptr<publish> ptr;
      typedef scrypt::sha1 id;
      struct record {
      ...
      publish( const boost::filesystem::path& dir );
      ~publish();
      ...
      bool fetch_index( uint32_t recnum, scrypt::sha1& id, record& m );
